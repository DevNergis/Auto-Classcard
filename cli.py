...
from learning_types import (
    memorization, recall, spelling, test, matching_game, matching_game_API, quiz_battle
)
...
def main(driver: WebDriver, loop: bool):
    ...
    driver.find_element(By.CSS_SELECTOR,
                        "#loginForm > div.checkbox.primary.text-primary.text-center.m-t-md > a").click()
    time.sleep(1)

    while temp_loop:
        time_1, time_2 = round(random.uniform(0.7, 1.3), 4), round(random.uniform(1.7, 2.3), 4)

        driver.get("https://www.classcard.net/Main")

        try:
            class_list_element = driver.find_element(By.CSS_SELECTOR,
                                                     "body > div.mw-1080 > div:nth-child(6) > div > div > div.left-menu > div.left-item-group.p-t-none.p-r-lg > div.m-t-sm.left-class-list")
            class_dict = {i: {"class_name": item.text, "class_id": item.get_attribute("href").split("/")[-1]} for
                          i, item in
                          enumerate(class_list_element.find_elements(By.TAG_NAME, "a")) if
                          item.get_attribute("href").split("/")[-1] != "joinClass"}
            ...
            sets_dict = {i: {"card_num": item.find_element(By.TAG_NAME, "a").find_element(By.TAG_NAME, "span").text,
                             "title": item.find_element(By.TAG_NAME, "a").text.replace(
                                 item.find_element(By.TAG_NAME, "a").find_element(By.TAG_NAME, "span").text, ""),
                             "set_id": item.find_element(By.TAG_NAME, "a").get_attribute("data-idx")} for i, item in
                         enumerate(sets)}
            ...
            driver.find_element(By.CSS_SELECTOR,
                                "body > div.test > div.p-b-sm > div.set-body.m-t-25.m-b-lg > div.m-b-md > div > a").click()
            driver.find_element(By.CSS_SELECTOR,
                                "body > div.test > div.p-b-sm > div.set-body.m-t-25.m-b-lg > div.m-b-md > div > ul > li:nth-child(1)").click()

            html = BeautifulSoup(driver.page_source, "html.parser")
            num_d = len(html.find("div", class_="flip-body").find_all("div", class_="flip-card")) + 1
            time.sleep(0.5)
            ...
            if ch_d == 1:
                ...
                classcard_api_post(user_id, sets_dict[choice_set_val]["set_id"], class_id, num_d, 1)
            elif ch_d == 2:
                print("리콜학습 API 요청 변조를 시작합니다.")
                classcard_api_post(user_id, sets_dict[choice_set_val]["set_id"], class_id, num_d, 2)
            elif ch_d == 3:
                print("스펠학습 API 요청 변조를 시작합니다.")
                classcard_api_post(user_id, sets_dict[choice_set_val]["set_id"], class_id, num_d, 3)
            elif ch_d == 4:
